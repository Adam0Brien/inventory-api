...
// We need a mock for ReporterResourceRepository, since Usecase.Check always calls it.
type MockRepo = mocks.MockedReporterResourceRepository

//func TestInventoryService_ReportResource_Upsert_Success(t *testing.T) {
...
//				Fields: map[string]*structpb.Value{
//					"os": structpb.NewStringValue("linux"),
//				},
//			},
//		},
//		WriteVisibility: pb.WriteVisibility_IMMEDIATE,
//	}
//
...
//		ReporterId:         "tester",
//		ReporterType:       "hbi",
//		ReporterResourceId: "v1",
//		ResourceType:       "host",
//		ResourceData:       map[string]any{"hostname": "example-host"},
//		InventoryId:        nil,
//	}
//
//	// Upsert will check for an existing resource first
//	mockRepo.
//		On("FindByReporterResourceIdv1beta2", mock.Anything, mock.Anything).
//		Return((*model.Resource)(nil), gorm.ErrRecordNotFound).
//		Once()
//
//	// Then attempt to create a new resource
//	mockRepo.
//		On("Create",
//			mock.Anything, // ctx
//			mock.AnythingOfType("*model.Resource"),
//			"rbac", // namespace
//			"",     // txid (empty in this test)
//		).
//		Return(expectedResource, nil).
//		Once()
//
//	// Final persistence step: record the reporter submission
//	mockRepo.
//		On("ReportResource",
//			mock.Anything,
//			"",
//			mock.AnythingOfType("*model.ReportedResource"),
//		).
//		Return(nil).
//		Once() // will not fail if the call is skipped
//
...
func TestRequestToResource_Success(t *testing.T) {
	req := &pb.ReportResourceRequest{
		...
		InventoryId:        ptr("9b8b5a02-4ac7-4d6c-a2c0-123456789abc"),
		Representations: &pb.ResourceRepresentations{
			Common: &structpb.Struct{
				Fields: map[string]*structpb.Value{
	...
	//assert.Equal(t, "reporter-1", resource.ReporterId)
	//assert.Equal(t, "hbi", resource.ReporterType)
	//assert.Equal(t, "inst-1", resource.ReporterInstanceId)
}
...
func TestRequestToResource_Error_MissingReporterInstanceId(t *testing.T) {
	req := &pb.ReportResourceRequest{
		ReporterType:       "hbi",
		ReporterInstanceId: "", // triggers error
		Representations: &pb.ResourceRepresentations{
			Common: &structpb.Struct{
				Fields: map[string]*structpb.Value{
					"workspace_id": structpb.NewStringValue("ws"),
...
func TestInventoryService_ReportResource_InvalidJsonObject(t *testing.T) {
	...
	// Struct that simulates unsupported/malformed shape
	badCommon := &structpb.Struct{
		Fields: map[string]*structpb.Value{
			"invalid": nil, // or use intentionally unsupported types
		},
	}
...
func TestInventoryService_DeleteResource_Success(t *testing.T) {
	...
	// Your usecase may check if the resource exists first
	mockRepo.
		On("FindByReporterData", mock.Anything, "tester", "abc123").
		Return(&model.Resource{}, nil).
		Once()

	// Updated mock to match the actual method signature (uuid.UUID, string)
	mockRepo.
		On("Delete",
		...
		Return(&model.Resource{
			ID:           uuid.New(), // or any dummy ID
			ResourceType: "host",     // or any test string
		}, nil).
		Once()
